...
Repeat the process, including the timing, exactly as above, except change the inner loop to iterate over the row index (the first index) and the outer loop iterate over the column index, making the access sequence row-major.
